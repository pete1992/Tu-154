-- flight_controls.lua
-- Tu-154M Flight Controls Logic | X-Plane 12 | SASL 3.19+
-- Batch DataRef setup, array access, optimized for all X-Plane 12 array Drefs
...
local props = {
    ...
    {"tire_vert_defl_arr","sim/flightmodel2/gear/tire_vertical_deflection_mtr","f"},
    ...
    {"thrust_rev_arr","sim/flightmodel2/engines/thrust_reverser_deploy_ratio","f"},
...
function update()
    ...
    -- Spoiler auto-deploy
    local gears = getArr(tire_vert_defl_arr,1)>0.01 and getArr(tire_vert_defl_arr,2)>0.01
    local idle  = true -- Insert engine idle logic if needed
    local rev   = getArr(thrust_rev_arr,0)>0.1 and getArr(thrust_rev_arr,2)>0.1
    ...
    if MASTER and pL and pR then
        ...
        if (sw==0 and get(flap_inn_L)<7 and get(flap_inn_R)<7 and getArr(tire_vert_defl_arr,1)<0.01 and getArr(tire_vert_defl_arr,2)<0.01) or sw==-1 then
    ...
    local fr = get(control_force_pos_rud)
    ...
    local rp = yaw_pos_act*25
    local ecoef_rud = (get(machno)<0.8) and 1 or 0.5
    local rL = 1-math.max(getArr(thrust_rev_arr,0)-0.5,0)*get(rpm_high_1)*0.015
    local rR = 1-math.max(getArr(thrust_rev_arr,2)-0.5,0)*get(rpm_high_3)*0.015
    if MASTER then
        set(rudder, rp*((rL+rR)*0.5)*ecoef_rud*(1-get(rudder_fail)))
