...
-- DataRefs for lamps and heating
...
defineProperty("slat_heating",  globalPropertyi("tu154ce/antiice/slat_heating"))      -- Added
defineProperty("wing_heating",  globalPropertyi("tu154ce/antiice/wing_heating"))      -- Added
...
-- Reset switches on cold & dark after sim load
...
local function update_gauges(dt)
    local s = get(stab_temp)
    local w = get(wing_temp)
    set(stab_temp, s + (get(stab_heat_t) - s) * dt * 5)
    set(wing_temp, w + (get(wing_heat_t) - w) * dt * 5)
end
