-- Battery logic: voltage, current, and temperature simulation for Tu-154M (SASL 3.19+)
...
local thermo           = 20                          -- battery temperature (°C)
...
function update()
    ...
    -- Calculate temperature-dependent maximum capacity for this frame
    local max_cap_temp = 45 + get(cockpit_temp)
    max_cap_temp = math.min(25, math.max(0, max_cap_temp))
    -- Reduce max cap if thermal runaway (kz_timer > 0)
    max_cap_temp = max_cap_temp - kz_timer

    -- Clamp battery capacity to current temp-dependent max
    bat_capacity = math.min(bat_capacity, max_cap_temp)
    ...
    -- Base voltage model (simplified Li-Ion curve)
    ...
    if bat_on == 1 then
        if get(bat_source) == 1 then
            ...
            if fail then
                bat_capacity = 0
                bat_volt     = 3
                -- max_cap_temp = 0   -- (nicht nötig, weil bat_capacity schon auf 0 geht)
            end
            bat_volt = math.max(bat_volt, get(bus_volt))
            bat_capacity = bat_capacity + passed * 0.01
            local charge_current = (max_cap_temp - bat_capacity) * BAT_CURRENT_COEF
                                  + interpolate(current_table, kz_timer)
            set(bat_amp_cc, charge_current)
        end

        -- Thermal runaway accumulation (KZ: zerstört die Batterie schrittweise)
        if runaway and kz_timer < 1800 then
            kz_timer = kz_timer + passed * KZ_TIMER_COEF
        end

        -- Clamp capacity in case of runaway and temp limits
        bat_capacity = math.max(0, math.min(bat_capacity, max_cap_temp))

        -- Apply voltage and temperature to DataRefs
        set(bat_volt_bus, bat_volt)
        thermo = 20 + get(bat_amp_cc) * 0.3
        set(bat_thermo, thermo)

    else
        -- Batteries OFF: set voltage, let temp slowly cool, show failures
        if fail then
            bat_capacity = 0
            bat_volt     = 3
        end
