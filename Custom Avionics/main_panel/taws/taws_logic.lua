...
defineProperty("rv_on", globalPropertyi("tu154ce/switchers/ovhd/rv5_2_on"))  
...
defineProperty("taws_cc", globalPropertyf("tu154ce/taws/taws_cc")) 
...
if MASTER then	
	...
	if not power then
		...
		if current_mode == 0 then set(mode_set, 4) end 
		...
		if (current_mode > 0 and current_mode < 4) or current_mode == 10 then
			if get(egpws_control) == 1 then set(mode_set, 5) end 
