...
if MASTER then		
	...
	appear_spd[1] = (precip_lvl - math.min(0.05 + math.abs(IAS) * 0.0005, 0.5)) * 0.3 
	if temperature < 0 then appear_spd[1] = -math.min(0.01 + math.abs(IAS) * 0.0005, 0.5) end
	appear_spd[2] = (precip_lvl - math.min(0.05 + math.abs(IAS) * 0.0005, 0.5)) * 0.1 
	...
	for i = 1, 2 do
		...
		for y = 1, 5 do
			...
			elseif wiper_L < wiper_L_last then 
			...
			elseif wiper_R < wiper_R_last then 
