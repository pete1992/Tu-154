...
defineProperty("tank_all", globalPropertyf("sim/flightmodel/weight/m_fuel_total")) 
...
defineProperty("apu_burn_fuel", globalPropertyi("tu154ce/elec/apu_burning_fuel")) 
...
defineProperty("gear_defl_L", globalPropertyf("sim/flightmodel2/gear/tire_vertical_deflection_mtr[1]")) 
defineProperty("gear_defl_R", globalPropertyf("sim/flightmodel2/gear/tire_vertical_deflection_mtr[2]")) 
...
function update()
	...
	if (tank1_qty < 3150 and get(fuel_porc_fail) == 0) or (get(fuel_porc) == 1 and power27 and not tank1_full) then porc_open = true 
	elseif tank1_full then porc_open = false end 
	...
	local spd = 1.55 - get(msl_alt) * 1.11 / 14000 
	local pump2L = get(pump_tank2_left_work) * spd * tank2L_rnd 
	local pump2R = get(pump_tank2_right_work) * spd * tank2R_rnd 
	local pump3L = get(pump_tank3_left_work) * spd * tank3L_rnd 
	local pump3R = get(pump_tank3_right_work) * spd * tank3R_rnd 
	local pump4 = get(pump_tank4_work) * spd 
